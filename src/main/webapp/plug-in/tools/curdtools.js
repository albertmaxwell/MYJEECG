 ...
 *   pcode:动态报表的code
 */
function popupClick(pobj,tablefield,inputnames,pcode,callback) {
    if(inputnames==""||pcode==""){
        alert($.i18n.prop('popup.param.error.msg'));
        return;
    }
                                ...
                                if(str.indexOf("undefined")==-1){
                                    ...
                                    ﻿	//update--begin--author:scott date:20171031 for:TASK #2385 online和代码生成器 一对多行popup多字段赋值问题解决-----------
                                        ...
                                        if(callback != undefined && callback != null){
                                            callback({value:str,selectedRow:selected});
                                        }else{
 ...
 *   params json参数
 */
function popupClickParam(pobj,tablefield,inputnames,pcode, params,callback) {
    if(inputnames==""||pcode==""){
        alert($.i18n.prop('popup.param.error.msg'));
        return;
    }
    ...
    if(typeof(windowapi) == 'undefined'){
        $.dialog({
            content: "url:cgReportController.do?popupParam&id="+pcode + "&params="+params,
            zIndex: getzIndex(),
            lock : true,
            title:$.i18n.prop('common.select'),
            width:800,
                    ...
                    if(tablefield != "" && tablefield != null){
                        ...
                        if(fields.length > 1){
                            ...
                            for(var i1=0;i1<fields.length;i1++){
                                ...
                                if(str.indexOf("undefined")==-1){
                                    if(inputs.length>1){
                                    ﻿	// update--begin--author:scott
                                        // date:20171031 for:TASK #2385
                                        // online和代码生成器
                                        // 一对多行popup多字段赋值问题解决-----------
 ...
 *   inputnames:对应字典CODE,当前需要回填数据的文本框名称.
 *   pcode:动态报表的code
 *   condition 条件 格式为：[{"column":"字段", "comp":"对比符", "value": "值"},{"sign":"条件符", "column":"字段", "comp":"对比符", "value": "值"}]
 *   注：1、条件中，目前仅支持and，相同字段可以存在多个
 *   	2、对比符支持>、=、<、>=、<=、<>、in，若为in，则value以英文逗号分隔
 *   例子：
 *   var condition = [{"column":"create_by", "comp":"in", "value": "system,admin"},{"column":"create_date", "comp":">=", "value": "2019-01-03 09:53:51"},{"column":"create_date", "comp":"<", "value": "2019-01-03 10:39:23"}];
 *   condition = JSON.stringify(condition);
 *   popupClickParamExt(obj, 'rrba15', 'a', 'test', condition, this.setTest);
 */
function popupClickParamExt(pobj,tablefield,inputnames,pcode, condition, callback) {
    var params = null;
    if(inputnames==""||pcode==""){
        alert($.i18n.prop('popup.param.error.msg'));
        return;
    ...
    if(condition != null && condition != "") {
        params = encodeURIComponent(condition);
    } else {
        params = "";
    }
    if(typeof(windowapi) == 'undefined'){
        $.dialog({
            content: "url:cgReportController.do?popupParam&id="+pcode + "&condition="+params,
            zIndex: getzIndex(),
            lock : true,
            title:$.i18n.prop('common.select'),
            width:800,
